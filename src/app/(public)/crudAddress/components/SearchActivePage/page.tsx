...
const SearchActivePage: FC = () => {
  ...
  const handleBackNavigation = () => {
    const id = JSON.parse(localStorage.getItem('editAddressId') || '""');
      ...
      if (response.data && response.data.documents) {
        setResults(
          response.data.documents.map((doc: any) => {
            const addressName = doc.road_address ? doc.road_address.address_name : doc.address_name;
            const detailAddress = doc.address
              ? `${doc.address.region_1depth_name} ${doc.address.region_2depth_name} ${doc.address.region_3depth_name} ${doc.address.main_address_no}${doc.address.sub_address_no ? '-' + doc.address.sub_address_no : ''}`
              : doc.address_name;
    ...
    <div className="flex flex-col items-center bg-gray-50 min-h-screen">
      ...
      {fullResponse && (
        <div className="w-full max-w-[430px] p-4 bg-white mt-4">
          ...
          <pre className="text-sm">{JSON.stringify(fullResponse, null, 2)}</pre>
        </div>
      )}
    </div>
  );
